...
namespace
{
    ...
    std::tuple<float, float> calculatePolygonCentroid(const std::vector<std::tuple<float, float>>& pts) 
    {
        if (pts.size() < 3) {
            ...
            if (!pts.empty()) {
                for (const auto& p : pts) { sum_x += std::get<0>(p); sum_y += std::get<1>(p); }
        ...
        signedArea *= 0.5f;
        if (std::abs(signedArea) < 1e-6) { return calculatePolygonCentroid({}); }
...
{
    if (!box.track || !box.track->history_pose.has_value()) return;
...
{
    if (box.type == object::ObjectType::TRACK) return;
    if (!box.pose) return;
